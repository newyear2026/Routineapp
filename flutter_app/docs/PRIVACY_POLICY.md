...
>
> **이 버전부터 앱에 Google AdMob 광고가 들어간다.** 그래서 «수집 안 함»이
> 더는 정확한 답이 아니다. 이 문서와 함께 Play Console의 다음 세 곳을
...
**개발자가 수집하는 정보는 없습니다.** 앱은 이름, 이메일, 전화번호, 위치, 연락처,
사진을 수집하지 않으며, 개발자에게 전송되는 데이터는 어떤 경우에도 없습니다.
...
저장됩니다. 이 데이터는 개발자를 포함한 누구에게도 전송되지 않으며, **광고 네트워크로도
전송되지 않습니다.**
...
| 인터넷 · 네트워크 상태 | **광고를 불러오는 데에만** 사용합니다. 루틴 데이터는 전송되지 않습니다. |
...
**The developer collects nothing.** The App does not collect names, email addresses,
phone numbers, location, contacts, or photos, and no data is transmitted to the
developer under any circumstances.
...
notifications) are stored **only in local storage on your device.** This data is never
transmitted to anyone, including the developer, and **is never sent to the ad network.**
...
| Internet · Network state | Used **only to fetch ads.** Your routine data is never transmitted. |
...
**El desarrollador no recopila nada.** La Aplicación no recopila nombres, correos
electrónicos, números de teléfono, ubicación, contactos ni fotos, y en ningún caso se
transmiten datos al desarrollador.
...
se guardan **únicamente en el almacenamiento local de tu dispositivo.** Estos datos nunca se
transmiten a nadie, incluido el desarrollador, y **nunca se envían a la red publicitaria.**
...
| Internet · Estado de la red | Se usan **únicamente para cargar anuncios.** Tus datos de rutinas nunca se transmiten. |
